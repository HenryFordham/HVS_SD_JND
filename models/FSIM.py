...
import math
from models.utils import abs, real, imag, downsample
...
def phasecong2(im):
    ...
    imagefft = torch.fft.fft2(im, dim=(-2, -1))
    ...
    for o in np.arange(norient):
        ...
        for s in np.arange(nscale):
            ...
            MatrixEO = torch.fft.ifft2(torch.complex(c[..., 0], c[..., 1]),
                                              dim=(-2, -1))
            MatrixEO = torch.stack((MatrixEO.real, MatrixEO.imag), -1)
            # MatrixEO = torch.fft.ifft(imagefft * filter.unsqueeze(-1).repeat(1, 1, 1, 1, 2), signal_ndim = 2)
...
#     import argparse
#     from models.utils import prepare_image
#
